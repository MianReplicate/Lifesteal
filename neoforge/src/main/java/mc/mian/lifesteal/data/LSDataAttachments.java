...
public class LSDataAttachments {
    ...
    public static final Supplier<AttachmentType<Integer>> HEALTH_DIFFERENCE = ATTACHMENT_TYPES.register(
            LSConstants.HEALTH_DIFFERENCE.getPath(), () -> AttachmentType.builder(() -> LifeSteal.config.startingHealthDifference.get()).serialize(Codec.INT.fieldOf("value")).copyOnDeath().build());
    public static final Supplier<AttachmentType<Long>> TIME_KILLED = ATTACHMENT_TYPES.register(
            LSConstants.TIME_KILLED.getPath(), () -> AttachmentType.builder(() -> 0L).serialize(Codec.LONG.fieldOf("value")).copyOnDeath().build());
}
